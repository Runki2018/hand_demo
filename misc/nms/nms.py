...
# todo : I ban the usage of the import of cpu_nms and gpu_nms
...
def oks_iou_21(g, d, a_g, a_d, sigmas=None, in_vis_thre=None):
    if not isinstance(sigmas, np.ndarray):
        # the bigger value of sigmas is, the more ambiguous the keypoint is
        sigmas = np.array([1.07, .89, .72, .52, .35,
                           .52, .62, .72, .62,
                           .52, .62, .72, .72,
                           .52, .62, .72, .72,
                           .57, .62, .72, .62]) / 10.0
...
def oks_nms_21(kpts_db, thresh, sigmas=None, in_vis_thre=None):
    """
    greedily select boxes with high confidence and overlap with current maximum <= thresh
    rule out overlap >= thresh, overlap = oks
    :param kpts_db
    ...
    while order.size > 0:
        ...
        oks_ovr = oks_iou_21(kpts[i], kpts[order[1:]], areas[i], areas[order[1:]], sigmas, in_vis_thre)
